...
export default function RamassagePlanningPage({ navigate, showNotification }) {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'confirmed':
        return { label: 'Confirmé', bg: 'bg-primary/20 text-primary border-primary/40 font-semibold' };
      case 'picked_up':
        return { label: 'Ramassé', bg: 'bg-emerald-500/20 text-emerald-500 border-emerald-500/40 font-semibold' };
      case 'cancelled':
        return { label: 'Annulé', bg: 'bg-rose-500/20 text-rose-500 border-rose-500/40 font-semibold' };
      default:
        return { label: 'En attente', bg: 'bg-amber-500/20 text-amber-500 border-amber-500/40 font-semibold' };
    }
  };
